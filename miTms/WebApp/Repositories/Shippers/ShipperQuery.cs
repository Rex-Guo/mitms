// <copyright file="ShipperQuery.cs" company="neozhu/MVC5-Scaffolder">
...
{
   public class ShipperQuery:QueryObject<Shipper>
    {
        public ShipperQuery WithAnySearch(string search)
        {
            if (!string.IsNullOrEmpty(search))
                And( x =>  x.Id.ToString().Contains(search) || x.Name.Contains(search) || x.Type.ToString().Contains(search) || x.ContactName.Contains(search) || x.ContactIdCard.Contains(search) || x.ContactMobileTelephoneNumber.Contains(search) || x.RegisteredAddress.Contains(search) || x.RegisteredCapital.ToString().Contains(search) || x.UnifiedSocialCreditldentifier.Contains(search) || x.UnifiedsocialDatetime.ToString().Contains(search) || x.Description.Contains(search) || x.LogoPicture.Contains(search) || x.CompanyId.ToString().Contains(search) || x.RegistrationDatetime.ToString().Contains(search) || x.UpdateTimeDatetime.ToString().Contains(search) || x.SynchronizationTime.ToString().Contains(search) );
            return this;
        }


		public ShipperQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
               foreach (var rule in filters)
               {
											...
											if (rule.field == "ContactName"  && !string.IsNullOrEmpty(rule.value))
						{
							And(x => x.ContactName.Contains(rule.value));
											...
											if (rule.field == "ContactIdCard"  && !string.IsNullOrEmpty(rule.value))
						{
							And(x => x.ContactIdCard.Contains(rule.value));
						}
            ...
            return this;
                 ...
                 public  ShipperQuery ByCompanyIdWithfilter(int companyid, IEnumerable<filterRule> filters)
         {
